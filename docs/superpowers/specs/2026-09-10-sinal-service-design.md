...
3. Calcula o valor do indicador e avalia a condição (`satisfeita`/não satisfeita) para **cada** índice `i` da série completa de cotações, não só os que caem nos últimos 24 meses — o cálculo do indicador nesse dia usa `cotacoes[:i+1]`, a série completa até ali, não truncada em 24 meses, e o rastreamento de `satisfeita`/não satisfeita também roda continuamente por toda a série, sem reiniciar no corte (assim um indicador de janela longa como SMA(200) funciona corretamente mesmo pra ocorrências logo no início da janela de 24 meses, e uma condição já verdadeira pouco antes do corte não é contada como transição nova no primeiro dia da janela).
4. Marca como "ocorrência" toda transição falso→verdadeiro (dia `i` satisfaz, dia `i-1` não satisfazia ou não existe) cujo `cotacoes[i].data_hora >= corte` — o corte de 24 meses só filtra quais transições **contam** como ocorrência registrada, não quais dias são avaliados no passo 3.
